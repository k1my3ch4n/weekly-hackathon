...
- [ ] **1-1** Node.js + TypeScript 프로젝트 초기화 (`package.json`, `tsconfig.json`)
- [ ] **1-2** 필수 패키지 설치
  ...
  - `dotenv`, `typescript`, `ts-node`, `@types/node`
- [ ] **1-3** `.env` / `.env.example` 파일 생성 (DISCORD_TOKEN, GROQ_API_KEY, OPENAI_API_KEY 등)
- [ ] **1-4** FSD 디렉토리 뼈대 생성
  ```
  src/app/ src/processes/ src/features/ src/entities/ src/shared/
  ```
- [ ] **1-5** `tsconfig.json` path alias 설정 (`@app/*`, `@shared/*` 등)
- [ ] **1-6** `nodemon` + `ts-node` 개발 서버 스크립트 설정

---

## Phase 2: shared 레이어 구축

- [ ] **2-1** `shared/types/index.ts` — 글로벌 타입 정의
  - `ChatMessage`, `SessionState`, `UserProfile`, `TranscriptionResult` 등
- [ ] **2-2** `shared/api/groqClient.ts` — Groq SDK 클라이언트 싱글톤 생성
- [ ] **2-3** `shared/api/openaiClient.ts` — OpenAI SDK 클라이언트 싱글톤 생성
- [ ] **2-4** `shared/utils/ffmpegWrapper.ts` — PCM → WAV 변환 헬퍼 함수
- [ ] **2-5** `shared/utils/logger.ts` — 공통 로거 (타임스탬프 + 레벨)

---

## Phase 3: entities 레이어 구축

- [ ] **3-1** `entities/user/index.ts` — 유저 도메인 모델 타입 정의 (`UserId`, `UserProfile`)
- [ ] **3-2** `entities/session/model/sessionStore.ts` — Zustand 세션 스토어 구현
  - 유저별 대화 히스토리 (`Map<UserId, ChatMessage[]>`) 저장
  - `addMessage`, `getHistory`, `clearSession` 액션 정의
- [ ] **3-3** `entities/session/index.ts` — 퍼블릭 API 배럴 익스포트
...
- [ ] **4-1** `features/audio-recorder/lib/silenceDetector.ts`
  - Discord `VoiceReceiver`에서 유저 오디오 스트림 구독
  - Silence Detection: 일정 시간(~800ms) 무음 감지 시 발화 종료 신호 발송
- [ ] **4-2** `features/audio-recorder/lib/pcmToWav.ts`
  - Opus 스트림 → PCM → WAV 파일 변환 (ffmpeg 래퍼 활용)
  - 임시 파일 경로 관리 (`/tmp` 또는 `os.tmpdir()`)
- [ ] **4-3** `features/audio-recorder/index.ts` — 배럴 익스포트

### 4-B. stt-transcriber (음성 → 텍스트)

- [ ] **4-4** `features/stt-transcriber/lib/transcribe.ts`
  - WAV 파일을 Groq Whisper API(`whisper-large-v3`)에 전송
  - 변환된 텍스트 반환 + 채팅창 출력 트리거
- [ ] **4-5** `features/stt-transcriber/index.ts` — 배럴 익스포트

### 4-C. ai-chat (LLM 답변 생성)

- [ ] **4-6** `features/ai-chat/lib/systemPrompt.ts`
  - 영어 튜터 역할 시스템 프롬프트 정의 (자연스러운 회화 유도, 문법 교정 포함)
- [ ] **4-7** `features/ai-chat/lib/generateReply.ts`
  - 세션 스토어에서 유저 대화 히스토리 조회
  - Groq Llama 3(`llama3-70b-8192`) API 호출 → 답변 생성
  - 새 메시지를 세션 스토어에 저장
- [ ] **4-8** `features/ai-chat/index.ts` — 배럴 익스포트

### 4-D. tts-speaker (텍스트 → 음성 송출)

- [ ] **4-9** `features/tts-speaker/lib/synthesize.ts`
  - OpenAI TTS API(`tts-1`, voice: `alloy` 또는 설정 가능) 호출
  - MP3 오디오 버퍼 반환
- [ ] **4-10** `features/tts-speaker/lib/playAudio.ts`
  - Discord `VoiceConnection`에 오디오 리소스 생성 후 재생
  - 재생 완료 이벤트 처리
- [ ] **4-11** `features/tts-speaker/index.ts` — 배럴 익스포트
...
- [ ] **6-1** `app/config.ts` — 환경 변수 로드 및 유효성 검사 (필수 키 누락 시 조기 종료)
- [ ] **6-2** `app/index.ts` — Discord 클라이언트 초기화
...
- [ ] **7-1** 슬래시 커맨드 등록 스크립트 (`scripts/deployCommands.ts`)
  - `/join` — 봇을 현재 음성 채널에 참여시키고 세션 시작
  - `/leave` — 봇이 음성 채널에서 나가고 세션 종료
- [ ] **7-2** `interactionCreate` 이벤트 핸들러
  - `/join` 처리: 음성 채널 연결 + `voiceConversation` 프로세스 시작
  - `/leave` 처리: 연결 해제 + 세션 클리어
  - `[다시 듣기 🔁]` 버튼 인터랙션 처리
- [ ] **7-3** 채팅 메시지 포맷 구현
